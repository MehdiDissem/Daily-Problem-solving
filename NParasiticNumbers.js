...
function calculateSpecial(n, base) {
    let d = 1; // number of digits
    let max = base - 1; // maximum digit
    while (true) {
      for (let i = 1; i <= max; i++) {
        const s = i.toString(base).padStart(d, '0');
        const p = parseInt(s.slice(-1) + s.slice(0, -1), base) * n;
        if (p.toString(base).slice(-1) === i.toString(base)) {
          return p.toString(base);
        }
      }
      d++;
      max = max * base + base - 1;
  ...
  //the tests won't work here, im using a brute force so the execution time will exceed 12000ms :(